...
def searchmain():
    windowsearch = tkinter.Toplevel()
    windowsearch.geometry('700x400')
    windowsearch.title("Rax m3u8视频搜索器")
    #名称label
    msg = '名称：'
    label = tkinter.Label(windowsearch, text=msg, font=('Arial', 10))
    label.place(x=20, y=15, anchor='nw')
    # 名称textbox
    global searchtext
    searchtext = tkinter.StringVar();
    searchtextbox = tkinter.Entry(windowsearch, show=None, textvariable=searchtext, font=('Arial', 10), width=20)

    v.set('')
    searchtextbox.place(x=75, y=15, anchor='nw')
    #   搜索按钮
    global searchbutton
    searchbutton = tkinter.Button(windowsearch, text='搜索', font=('Arial', 10), width=10, command=searchme)
    searchbutton.place(x=250, y=10, anchor='nw')
    ...
    resulttextbox.place(x=0, y=40, anchor='nw')
    # 声明label
    msg2 = '声明：本软件不储存任何视频资源，所有搜索结果均来自于互联网，与本软件无关，请勿使用本软件于非法用途。'
    label2 = tkinter.Label(windowsearch, text=msg2, font=('Arial', 10))
    label2.place(x=10, y=375, anchor='nw')
